...
I thought that this command would have something to do with listing running services on the machine.

### Purpose

This command does display the running services on the machine.

### Result

Running this command showed me that 16 services were active and running. 

### SOC Relevance

It is important to know how to find what services you have running on your machine when it comes time to troublshoot problems. This command could point you to exactly the troubled service.
...
My prediction was that this command would show me whether ssh was running or not.

### Purpose

This command tells you the status of ssh and whether it's loaded, active, and it's triggers.

### Result

Running this command showed me that ssh is not yet active. 

### SOC Relevance

It's important for security analysts to know how to check whether ssh is an active service on their machines. Unknown exposed ssh could be a huge vulnerability on a system.
...
I thought that this command would provide me with more information into what the ssh service is doing on the machine.

### Purpose

This command is used to show the logs that are generated by the ssh service.

### Result

Running this command showed that there were no log entries for the ssh service. We have not yet run ssh on this machine.

### SOC Relevance

The importance of this command cannot be over stated. Being able to see the activity happening in the ssh service can allow an analyst to closely monitor this attack vector if left unsecured.
...
I thought these commands would give me more information into whether the ssh service is active and enabled.

### Purpose

The is-active command asks is the service running right now?
The is-enabled command asks if it will restart after reboot?

### Result

Running these commands showed me that the ssh service is both inactive and disabled.

### SOC Relevance

These are more useful commands in learning more about how the ssh service is behaving on your machine.
...
I figured this command would give me the status on whether elasticsearch is running or enabled.

### Purpose

This command is used to check the status of elasticsearch and show whether it's active and running or not.

### Result

Elasticsearch is not installed yet so running this command returned an expected error that states Elasticsearch can't be found.

### SOC Relevance

This goes back to reiterate the importance of knowing how to check the status and health of the running services on your machine. If you're troublshooting a problem with elastic, the first thing you could do to check the status of the service. Is it running?
